"""Sarah Agent integration adapter (stubbed)."""
...
class SarahAgentAdapter(IntegrationAdapterContract):
    """Adapter for delivering approved trends to Sarah Agent."""
    ...
    async def deliver(self, payload: IntegrationPayload) -> IntegrationResult:
        ...
        if not settings.sarah_agent_base_url:
            logger.warning("sarah_agent_not_configured")
            return IntegrationResult(
                success=False,
                error="Sarah Agent base URL not configured",
            )

        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{settings.sarah_agent_base_url}/api/v1/trends/ingest",
                    json={
                    ...
                    headers={
                        "X-Api-Key": settings.sarah_agent_api_key or "",
